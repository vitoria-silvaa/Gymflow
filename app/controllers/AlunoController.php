...
/* CADASTRAR */ elseif ($acao === 'cadastrar') {
    $erro = '';
    $dados = $_POST;
        ...
        if (
            ...
            if ($cpfExiste) {
                ...
                if ($erroModel === '') {
                    header(
                        "Location: $baseUrl"
                            . "?acao=visualizar&id=$novoAlunoId"
                            . "&msg=cadastrado"
                    );
                    exit;
                }
...
/* EDITAR */ elseif ($acao === 'editar') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (
            ...
            if ($cpfExiste) {
                $erro = 'Este CPF pertence a outro aluno.';
            } elseif ($emailExiste) {
...
/* VISUALIZAR */ elseif ($acao === 'visualizar') {
    $id = (int) ($_GET['id'] ?? 0);
...
/* MATRICULAR */ elseif ($acao === 'matricular') {
    $alunoId = (int) ($_POST['aluno_id'] ?? 0);
    $planoId = (int) ($_POST['plano_id'] ?? 0);
    $dataInicio = $_POST['data_inicio'] ?? '';

    $desconto = str_replace(
        ',',
        '.',
        $_POST['desconto'] ?? '0'
    );
    ...
    if (
        ...
        $_SESSION['erro_aluno'] =
            'Preencha corretamente os dados da matrícula.';
    } else {
...
/* PAGAR */ elseif ($acao === 'pagar') {
    ...
    $_SESSION['erro_aluno'] =
        'A mensalidade não foi encontrada ou já está paga.';
